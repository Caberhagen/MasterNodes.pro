...
//            $coinData = [];
            ...
            if (isset($one['ws'])) {
                $client = new Client();
                $getcoin = $client->request(
                    'GET', 'http://' . $one['ws'] . '/' . strtolower($one['coin']) . '-stats.json'
                );
                $coinData = $getcoin->getBody();
            ...
            if (isset($one['mnpstat'])) {
                $setData['mnpstat'] = $one['mnpstat'];
                if ($one['mnpstat'] === 'maint') {
                    $setData['realRoi'] = -1;
                }
            }
            if (isset($one['off'])) {
